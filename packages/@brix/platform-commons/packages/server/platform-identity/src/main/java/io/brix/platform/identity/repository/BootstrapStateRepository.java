...
/** Repository for the singleton bootstrap state row. */
@Repository
public interface BootstrapStateRepository extends JpaRepository<BootstrapState, Long> {

    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT s FROM PlatformIdentityBootstrapState s WHERE s.id = :id")
    Optional<BootstrapState> findByIdForUpdate(@Param("id") Long id);
}
