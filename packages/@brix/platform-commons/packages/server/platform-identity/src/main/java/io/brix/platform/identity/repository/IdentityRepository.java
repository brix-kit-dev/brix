...
public interface IdentityRepository extends JpaRepository<Identity, Long> {
     ...
     */
    @Query("SELECT i FROM PlatformIdentityIdentity i WHERE LOWER(i.email) = LOWER(:email)")
    ...
    @Query("SELECT CASE WHEN COUNT(i) > 0 THEN true ELSE false END "
            + "FROM PlatformIdentityIdentity i WHERE LOWER(i.email) = LOWER(:email)")
     ...
     */
    @Query("SELECT i FROM PlatformIdentityIdentity i WHERE i.status = 'ACTIVE' AND i.emailVerified = false")
     ...
     */
    @Query("SELECT i FROM PlatformIdentityIdentity i WHERE i.lastLoginAt < :since OR i.lastLoginAt IS NULL")
     ...
     */
    @Modifying
    @Query("UPDATE PlatformIdentityIdentity i SET i.lastLoginAt = :loginTime, i.updatedAt = :loginTime "
            + "WHERE i.id = :id")
    ...
    @Query("UPDATE PlatformIdentityIdentity i SET i.passwordHash = :hash, i.passwordMustChange = false, "
     ...
     */
    @Modifying(clearAutomatically = true)
    @Query("UPDATE PlatformIdentityIdentity i SET i.tokenVersion = i.tokenVersion + 1, "
            + "i.updatedAt = :updatedAt WHERE i.id = :id")
     ...
     */
    @Query("SELECT i FROM PlatformIdentityIdentity i WHERE " +
