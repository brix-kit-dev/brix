...
-- 身份域（identity_*）表由 brix-app-identity 模块的 V2 迁移创建。
...
-- 初始化种子数据
-- ============================================================================
-- P1-6 ARCHITECTURE NOTE: SUPER_ADMIN is a *platform-level* role stored in
-- sys_platform_admin.role (managed by platform-tenant module). It must NOT be
-- seeded here as a tenant business role in auth_role.
--
-- auth_role holds tenant-scoped RBAC roles (TENANT_ADMIN, USER, custom roles).
-- Platform admin roles are code constants defined in PlatformAdminRole enum and
-- never stored as rows in this table. Seeding SUPER_ADMIN here conflates the
-- two models and risks tenant logic treating a platform concept as a business role.
--
-- REMOVED: ('role-super-admin', 'SUPER_ADMIN', ...) — see sys_platform_admin instead.
